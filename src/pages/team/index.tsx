...
const TeamPage: React.FC = () => {
  ...
  const [updateName] = useUpdateTeamMutation({
    ...
    onCompleted: completedData => {
      history.push({
        pathname: `/team/${encode(completedData.updateTeam.id)}/${
          completedData.updateTeam.name
        }`
      });
    }
  });
  ...
  const [deleteProject] = useDeleteTeamProjectMutation({
    onCompleted: deleteData => {
      history.push({
        pathname: `/project/${encode(deleteData.deleteTeamProject.id)}/${
          deleteData.deleteTeamProject.name
        }`
      });
    },
    refetchQueries: [
          ...
          {data.getUserTeam.projects.map((project, idx) => (
            <li key={`team-project-${project.id}`}>
              <div
                ...
                onClick={() => {
                  history.push(
                    `/project/${encode(project.id)}/${project.name}`
                  );
                }}
              >
                {project.name}
              </div>
              <Button onClick={handleDeleteProject} id={`${project.id}`}>
                Delete Project
              </Button>
            </li>
          ))}
  ...
  const renderTeamName = () => {
    if (data) {
      return <div className={styles.teamName}>{data.getUserTeam.name}</div>;
    }
  };
        ...
        <ul className={styles.teamMembers}>
          {data.getUserTeam.members.map((member, idx) => (
            <li key={`team-member-${member.id}`}>
              {member.username}
              <Button onClick={handleDeleteMember} id={`${member.id}`}>
                Delete Member
              </Button>
            </li>
          ))}
